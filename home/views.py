...
def agregar_prestamo(request):
	...
	if request.method=='POST':
		...
		if formulario.is_valid() and formset.is_valid():			
			...
			if p:
			# documentar	
				OBJETO = Aprendiz.objects.get(identificacion__iexact= request.POST['aprendiz'] )
				prest.aprendiz = OBJETO
				prest.save()
				for j in codigos:
					j.prestamo = prest
					j.save()	
...
def busqueda(request):
	if request.is_ajax():
		proyectos = Aprendiz.objects.filter(identificacion__iexact = request.GET['nombre'] ).values('nombre', 'ficha','id')
		# proyectos = Aprendiz.objects.filter(identificacion__startswith= request.GET['nombre'] ).values('nombre', 'id')
		#proyectos = Aprendiz.objects.get(identificacion__iexact= request.GET['nombre'] ).values('nombre', 'id')
...
def get1(request):
	if request.is_ajax():
		proyectos = Ficha.objects.filter(identificacion__iexact = request.GET['nombre'] ).values('nombre','id')
		# proyectos = Aprendiz.objects.filter(identificacion__startswith= request.GET['nombre'] ).values('nombre', 'id')
		#proyectos = Aprendiz.objects.get(identificacion__iexact= request.GET['nombre'] ).values('nombre', 'id')
		return HttpResponse( json.dumps( list(proyectos)), content_type='application/json' ) 
	else:
...
# # 	elem=request.GET.get('id')
# # 	almacen=Material.objects.filter(id=elem)	
# # 	data=serializers.serialize('json',almacen,
# # 		fields=('numero_serie','codigo_sena'))
# # 	print("hola muestro esro",almacen)
# # 	return HttpResponse(data,content_type='application/json')
# documentar
# def get1(request,**kwargs):
#  	id_apren=request.GET.get('aprendiz')
#  	nose=Ficha.objects.filter(ide__iexact=id_apren)	
#  	data=serializers.serialize('json',nose,
#  		fields=('numero_ficha'))
#  	print("hola muestro esro",nose)
#  	return HttpResponse(data,content_type='application/json')
...
def editar_prestamo(request,id_prest):
	...
	Detalle_PrestamoFormSet = inlineformset_factory(Prestamo,Detalle_Prestamo,extra=0, form=editar_DPrestamoF,can_delete=True)
	if request.method=='POST':
		formulario=agregar_prestamoF(request.POST,instance=p)
		formset=Detalle_PrestamoFormSet(request.POST,instance=p)
		if formulario.is_valid() and formset.is_valid():
			OBJETO = Aprendiz.objects.get(identificacion__iexact= request.POST['aprendiz'] )
			formulario.aprendiz = OBJETO					
			formulario.save()			
			formset.save()			
